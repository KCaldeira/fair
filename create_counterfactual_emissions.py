#!/usr/bin/env python3
"""
Script to create counterfactual emissions file based on 1975 carbon intensity scenario.

This script:
1. Reads the Excel file to get CO2 emissions adjustment data
2. Reads the baseline emissions_ssp245.csv file
...
def create_counterfactual_emissions():
    """
    Create counterfactual emissions file with adjusted CO2 emissions.
    """
    ...
    # Read the baseline emissions file
    ...
    co2_row = df_baseline[df_baseline['Variable'] == 'Emissions|CO2'].copy()
    
    if len(co2_row) == 0:
        raise ValueError("Could not find 'Emissions|CO2' in the baseline emissions file")
    
    print("Found CO2 emissions data in baseline file")
    
    # Create a copy of the baseline file for modification
    df_counterfactual = df_baseline.copy()
    ...
    for _, row in adjustment_data.iterrows():
        ...
        if str(year) in df_counterfactual.columns:
            ...
            print(f"  Warning: Year {year} not found in baseline emissions file")
    
    # Save the modified emissions file
    output_file = "inputs/emissions_ssp245_counterfactual_1975.csv"
    df_counterfactual.to_csv(output_file, index=False)
    print(f"\nSaved counterfactual emissions file: {output_file}")
...
if __name__ == "__main__":
    create_counterfactual_emissions()
